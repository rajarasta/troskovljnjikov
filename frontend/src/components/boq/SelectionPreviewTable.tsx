...
export default function SelectionPreviewTable({ items, color }: SelectionPreviewTableProps) {
  ...
  return (
    <table className={`w-full text-[11px] border-collapse ${BORDER_CLASSES[color]}`}>
      <thead>
        <tr className="bg-bg-secondary/60">
          <th className="px-1.5 py-1 text-left text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[40px]">#</th>
          <th className="px-1.5 py-1 text-left text-[10px] font-semibold uppercase tracking-wider text-text-muted">Opis</th>
          <th className="px-1.5 py-1 text-left text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[30px]">JM</th>
          <th className="px-1.5 py-1 text-right text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[50px]">Kol.</th>
          <th className="px-1.5 py-1 text-right text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[60px]">JC</th>
          <th className="px-1.5 py-1 text-right text-[10px] font-semibold uppercase tracking-wider text-text-muted w-[60px]">Ukupno</th>
        </tr>
      </thead>
      <tbody>
        {items.map((item) => {
          ...
          if (isHeader) {
            return (
              <tr key={item.id} className="bg-accent-cyan/10">
                <td className="px-1.5 py-1 text-text-muted">{item.item_number ?? ""}</td>
          ...
          return (
            <tr key={item.id} className="border-t border-border-default/30 hover:bg-bg-hover/30">
              <td className="px-1.5 py-1 text-text-muted align-top whitespace-nowrap">{item.item_number ?? ""}</td>
