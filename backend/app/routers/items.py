...
def match_items(
    ...
    # Composite detection path: when file context is provided
    if req.file_id and req.start_row is not None and req.end_row is not None:
        return _match_with_row_lookup(req, db)

    # Standard flat matching
    return _flat_match(req, db)


# ── Helpers ───────────────────────────────────────────────────────────


def _build_stats(prices: list[float]) -> dict[str, Any]:
    return {
        "count": len(prices),
...
def _process_hits(
    ...
    for hit, db_id in item_hits:
        ...
        if quantity is not None:
            qty_comp = _quantity_comparison(quantity, item.quantity or 0)
    ...
    for hit, db_unit_id in unit_hits:
        ...
        qty_comp = None
        if quantity is not None:
            qty_comp = _quantity_comparison(quantity, 1)
        results.append(MatchResult(
...
def _flat_match(req: MatchRequest, db: Session) -> MatchResponse:
    ...
    results, prices = _process_hits(hits, req.threshold, req.quantity, db)
    return MatchResponse(matches=results, stats=_build_stats(prices))


def _match_with_row_lookup(req: MatchRequest, db: Session) -> MatchResponse:
    ...
    for sub_item in composite_subs:
        ...
        group_results, group_prices = _process_hits(
        ...
        groups.append(MatchGroup(
            ...
            stats=_build_stats(group_prices),
        ))

    return MatchResponse(
        matches=[],
        stats=_build_stats(all_prices),
        groups=groups,
        is_composite=True,
...
def price_history(
    ...
    # Sort by file_date descending (newest first), nulls last
