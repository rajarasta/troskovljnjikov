...
export default function SpreadsheetView({ items }) {
  ...
  const getRowHeightForItem = (item) => {
    ...
    // For better readability, let's calculate a more balanced approach
    const descLength = item.description.length;
    const baseMinHeight = 40; 
    
    // If we have very long descriptions or text with multiple lines,
    // compute the actual height needed for full visibility  
    if (descLength > 300) {
      return `${Math.min(250, Math.max(baseMinHeight, descLength / 1.5))}px`;
    } else if (descLength > 150) {
      return `${Math.min(200, Math.max(baseMinHeight, descLength / 2))}px`;
    } else if (descLength > 80) {
      return `${Math.min(140, Math.max(baseMinHeight, descLength / 1.75))}px`;
    }
    
    // For shorter descriptions or when no description is provided
    return '40px';
  };
        ...
        <tbody>{Array.isArray(items) && items.map((item, idx) => (
          ...
          >
            {columns.map(col => {
              ...
              return (<td
                key={col.key}
                className={col.key === 'description' ? 'description-cell' : ''}
                style={{
                  ...cellStyle,
                  minHeight: getRowHeightForItem(item),
                  height: getRowHeightForItem(item)
                }}>{value !== null && value !== undefined ? String(value) : ''}</td>);
            })}
          </tr>
        ))}</tbody>
      </table>
